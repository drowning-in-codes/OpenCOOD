...
from opencood.models.fuse_modules.range_attn_fusion import RangeAttentionBlock
...
class RangeAttentionFusion(nn.Module):
    def __init__(self, input_dim: int, model_cfg: dict, feature_pyramid: bool = True,feature_len: int = 3):
        super().__init__()
        ...
        if feature_pyramid:
            ...
            for idx in range(min(num_levels,feature_len)):
                self.setattr(f"smooth_{idx}",nn.Conv2d(num_upsample_filters[idx], num_upsample_filters[idx], kernel_size=1, bias=True))
            ...
            if len(self.deblocks) > 0:
                if self.feature_pyramid:
                    if i == 0:
                        ...
                        _,_,h,w = last_decode_feature.shape
                        decode_out = self.deblocks[i](x_fuse,)
                        F.upsample(decode_out,size=(h,w),mode="bilinear")+last_decode_feature
                ups.append(self.deblocks[i](x_fuse))
            else:
                ups.append(x_fuse)
