...
def test_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="specify epoch for evaluation",
    )
    parser.add_argument(
        "--show_vis",
        action="store_true",
...
def main():
    ...
    epoch, model = train_utils.load_saved_model(saved_path, model, opt.last_epoch)
    model.eval()
    if "wild_setting" in hypes and "hypes" in hypes["wild_setting"] and hypes["wild_setting"]["loc_err"]:
    ...
    eval_utils.eval_final_results(
        ...
        communication_rates=(
            communication_rates if opt.fusion_method == "intermediate" else 1
        ),
    )
    if opt.show_sequence:
        vis.destroy_window()
