...
class coComm(nn.Module):
    def __init__(self, H=None, W=None, ratio: float = .9, multi_scale=True, input_dim: int = 384, args=None,
                 fusion_args=None,with_comm=False) -> None:
        super().__init__()
                ...
                if self.with_comm:
                    if idx == 0:
                        ...
                        for batch in range(B):
                            V = record_len[batch]
                            confidence_map, _ = batch_confidence_maps[batch].sigmoid().max(dim=1,
                                                                                           keepdim=True)  # confidence map
                            ...
                            if self.training:
                                ...
                                ones_fill = torch.ones(V, K, dtype=communication_maps.dtype,
                                                       device=communication_maps.device)
                                communication_mask = torch.scatter(communication_mask, -1, indices, ones_fill).reshape(V, 1,
                                                                                                                       H, W)
                            elif self.threshold:
                                ones_mask = torch.ones_like(communication_maps).to(communication_maps.device)
                                zeros_mask = torch.zeros_like(communication_maps).to(communication_maps.device)
                                communication_mask = torch.where(communication_maps > self.threshold, ones_mask,
                                                                 zeros_mask).reshape(V, 1, H, W)
                            else:
