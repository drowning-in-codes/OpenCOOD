...
class V2XViTDA(nn.Module):
    def __init__(self, args):
        ...
        self.cls_head = nn.Conv2d(128 * 2, args['anchor_number'],  # classification
                                  kernel_size=1)
        self.reg_head = nn.Conv2d(128 * 2, 7 * args['anchor_number'],  # bbox regression
                                  kernel_size=1)
    ...
    def forward(self, data_dict):
        ...
        features_masks = data_dict['feature_masks']
        ...
        for i in range(b):
            cur_features_batch, cur_feature_mask = spatial_features_2d[i], \
                features_masks[i]
            ego_feature_mask = cur_feature_mask[0]
            ego_feature = cur_features_batch[0]
            ego_feautre = ego_feature[:int(ego_feature_mask[0]), :int(ego_feature_mask[1]),
            ...
            prior_encoding = prior_encoding.repeat(1, 1, 1, regroup_feature.shape[3], regroup_feature[4])
