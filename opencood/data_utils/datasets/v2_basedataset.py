...
from opencood.data_utils.augmentor.v2_data_augmentor import DataAugmentor
from opencood.hypes_yaml.yaml_utils import load_yaml
from opencood.utils.pcd_utils import downsample_lidar_minimum
from opencood.utils.v2_transformation_utils import x1_to_x2,dist_two_pose
...
class BaseDataset(Dataset):
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            data[cav_id]['folder_name'] = \
                cav_content[timestamp_key_delay]['lidar'].split(os.path.sep)[-3]
    ...
    def calc_dist_to_ego(self, scenario_database, timestamp_key):
        ...
        for cav_id, cav_content in scenario_database.items():
            cur_lidar_pose = \
                load_yaml(cav_content[timestamp_key]['yaml'])['lidar_pose']
            print("cur_lidar_pose",cur_lidar_pose,"ego_lidar_pose",ego_lidar_pose)
    ...
    @staticmethod
    def extract_transformations(matrix):
        # Extract translation
        x = matrix[0, 3]
        y = matrix[1, 3]
        z = matrix[2, 3]
        # Ensure rotation matrix is valid
        if not np.allclose(np.linalg.det(matrix[:3, :3]), 1):
            raise ValueError("The provided matrix does not represent a valid rotation.")
        # Extract rotation
        # 提取旋转矩阵
    ...
    def add_loc_noise(self, pose, xyz_std, ryp_std):
        ...
        print("----------",pose)
        if len(pose[0]) == 4:
            pose[0][3] += xyz_noise[0]
            pose[1][3] += xyz_noise[1]
            pose[2][3] += xyz_noise[2]
            # pose = self.extract_transformations(pose)
            return pose
