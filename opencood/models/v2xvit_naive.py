...
class V2XViNaive(nn.Module):
    def __init__(self):
        super().__init__()
        self.save_feature_flag = False
        if 'save_feature' in args:
    ...
    def forward(self, data_dict):
        ...
        for i in range(b):
            cur_feature_batch, cur_feature_mask = spatial_features_2d[i], feature_masks[i]
            ...
            cav_feature_mask = cur_feature_batch[1:record_len[i]]
            if cav_feature.shape[0] > 0:
                cav_feature = cav_feature[:, :int(cav_feature_mask[0, 0]),
                              :int(cav_feature_mask[0, 1]),
                              :int(cav_feature_mask[0, 2])]
                cav_feature = F.interpolate(cav_feature, [ego_feature.shape[1],
                                                          ego_feature.shape[2]],
                                            mode='bilinear',
                                            align_corners=False)
