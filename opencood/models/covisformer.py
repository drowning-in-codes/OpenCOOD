...
class UpMerge(nn.Module):
    ...
    def __init__(self, dim, scale_factor=2, norm_layer=nn.BatchNorm2d,drop_rate=0.8):
        super().__init__()
...
class DownMerge(nn.Module): # [B,L,C,H,W] -> [B,L,C,H//2,W//2]
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm,downsample_rate=2,drop_rate=0.8):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def forward(self, x, context=None,mask=None): # -> b l x y w1 w2 d
        ...
        if self.sr_ratio is None or self.sr_ratio[0] == 1 or self.sr_ratio[1] == 1:
            ...
            dim_h = math.floor(math.sqrt(D))
            dim_w = dim // dim_h
            x_ = x.reshape(B, N, dim_h, dim_w)
...
class CoVisFormer(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_stages):
            ...
            patchMerge = DownMerge(embed_dim[i],downsample_rate=scale_factor[i]) if scale_factor[i] != 1 else nn.Identity()
            """
            upscale to keep the same resolution
            """
            up_patch_merge = UpMerge(embed_dim[i],scale_factor=scale_factor[i]) if scale_factor[i] != 1 else nn.Identity()
