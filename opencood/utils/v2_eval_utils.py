...
def eval_final_results(result_stat, save_path, range="",loc_error=0,heading_error=0,epoch=None):
    dump_dict = {}
    file_name = 'eval.yaml' if range == "" else range+'_eval.yaml'
    if epoch:
        file_name = f'eval_{epoch}.yaml' if range == "" else range+f'_eval_{epoch}.yaml'
