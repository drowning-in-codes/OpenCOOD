...
class PointPillarErmvp(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        idx = torch.arange(H * W).repeat(N, 1, 1).permute(2, 0, 1).to('cuda') # H*W N 1
        # src:N,B,C
        # loss is not used????
        src, sample_reg_loss, sort_confidence_topk, pos_embed = self.sampler(spatial_features_2d, idx, None,
                                                                 dis_priority)
        # 通过一个卷积层得到reweight之后的特征.  h*w bs c
        # # # B N C
        src = src.permute(1, 0, 2) # bs h*w->max_sample_len c
        _, s_len, _ = src.shape
