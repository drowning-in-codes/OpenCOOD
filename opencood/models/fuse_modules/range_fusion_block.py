...
class CrossAgentAttention(nn.Module):
    ...
    def forward(self, batch_spatial_feature, distance=None, mode="context"):
        ...
        if distance is not None:
            ...
            distance = rearrange(distance, "b (c h) -> 1 h b c", h=self.heads) # (2,64) -> 1 4 2 64
        q, k, v = map(
        ...
        if distance is not None:
            if mode == "context":
                context = torch.matmul(
                    #  35200, 4, 2, 64       1 4 64 2
                    q, distance.transpose(-1, -2)
                )  # [H*W,head,cav_num,cav_num]
                score += context
            else:
                score += distance
        attn = F.softmax(score, -1)  # [H*W,head,cav_num,cav_num]
        out = torch.matmul(
            attn, v
...
class LocalRangeAttentionBlock(nn.Module):
    ...
    def forward(self, spatial_features):  # spatial_features [1,C,H,W]
        ...
        self.att_maps_a = att_maps_a
        self.att_maps_b = att_maps_b
        # print(att_maps_a.shape)
        # plt.imshow(att_maps_a.cpu().squeeze().numpy(),vmin=0,vmax=1)
        # plt.show()
        # plt.imshow(att_maps_b.cpu().squeeze().numpy(),vmin=0,vmax=1)
        # plt.show()
...
class LocalAttention(nn.Module):
    ...
    def __init__(self, input_dim, depth: int = 1):
        super().__init__()
...
class RangeAttentionBlock(nn.Module):
    ...
    def forward_3(self, spatial_features, record_len, distance=None):
        pass
    ...
    def forward_(self, spatial_features, record_len, distance=None):
        ...
        for idx, batch_spatial_feature in enumerate(split_x):  # [2,C,H,W]
            ...
            out_feat = ego_feat+local_feat+agent_feat
            out_feature = torch.max(out_feat, dim=0, keepdim=True)[0]
    ...
    def forward(self, spatial_features, record_len, distance=None):
        ...
        for idx, batch_spatial_feature in enumerate(split_x):  # [2,C,H,W]
            ...
            ego_feat = batch_spatial_feature[0:1]  # [1,C,H,W]
            out_feat = ego_feat + local_feat + agent_feat
            out_feature = torch.sum(out_feat, dim=0, keepdim=True)
...
#
# class CrossAgentAttention(nn.Module):
#     def __init__(self, input_dim, heads: int = 5) -> None:
#         super().__init__()
#         inner_dim = input_dim * heads
#         self.inner_dim = inner_dim
#         self.heads = heads
...
#
# class CrossAgentWiseAttention(nn.Module):
#     def __init__(self, input_dim, depth: int = 5) -> None:
#         super().__init__()
#         self.blks = nn.ModuleList()
#         for _ in range(depth):
#             caa_blk = CrossAgentWiseAttentionBlock(input_dim)
...
#     """
#
#     def __init__(self, input_dim, depth: int = 3):
#         super().__init__()
#         self.lsgs = nn.ModuleList()
#         for i in range(depth):
#             lra = LocalInvertedAttention(input_dim)
