...
def train_parser():
    ...
    parser.add_argument("--model", default="", help="for fine-tuned training path")
    parser.add_argument(
...
def convert_to_float32(output_dict):
    """
    将 output_dict 字典中的 tensor 数据类型转换为 float32.

    Args:
        output_dict (dict): 包含 tensor 的字典.

    Returns:
        dict: 包含 float32 类型 tensor 的新字典.
    """
    float32_dict = {}
...
def main():
    ...
    source_opencood_train_dataset = build_dataset(
        hypes, visualize=False, train=True, isSim=True
    )
    # Modify the root to the target domain
    hypes["root_dir"] = hypes["root_dir_target"]
    target_opencood_train_dataset = build_dataset(
        hypes, visualize=False, train=True, isSim=False
    )

    source_train_loader = DataLoader(
        source_opencood_train_dataset,
    ...
    opencood_validate_dataset = build_dataset(
        hypes, visualize=False, train=False, isSim=False
    )
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for iteration, source_batch_data in enumerate(source_train_loader):
            ...
            if not opt.half:
                ...
                final_loss = criterion(
                    ouput_dict, source_batch_data["ego"]["label_dict"]
                )
            else:
            ...
            criterion.logging_da(
                epoch, iteration, len(source_train_loader), writer, da_loss, pbar=pbar2
            )
        ...
        if (epoch + 1) % hypes["train_params"]["eval_freq"] == 0:
            ...
            print(
                "At epoch %d, the validation loss is %f" % ((epoch + 1), valid_ave_loss)
            )

            writer.add_scalar("Validate_Loss", valid_ave_loss, (epoch + 1))

        if (epoch + 1) % hypes["train_params"]["save_freq"] == 0 or ((epoch + 1) > 30 and (epoch + 1) % 3 == 0):
            torch.save(
                model.state_dict(),
