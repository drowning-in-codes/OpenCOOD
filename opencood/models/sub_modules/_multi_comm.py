...
class MultiRangeComm(nn.Module):
    ...
    def __init__(self, input_dim, cls_head, config, sim_type=0, H=None, W=None):
        ...
        self.threshold = config['threshold'] if config is not None and 'threshold' in config else 0.1
    ...
    def gen_comm_map(self, near_range_featm, far_range_feat):
        ...
        far_confidence_map, _ = far_conf_map.sigmoid().max(dim=1, keepdim=True)  # confidence  # [V,1,H,W]
    ...
    def forward(self, batch_features):
        ...
        for conv, n_conv, f_conv in zip(self.down_convs, self.n_convs, self.f_convs):
            ...
            if self.smooth:
                comm_map = self.gaussian_filter(comm_map)
        ...
        comm_maps = self.conv_g(all_comm_maps)
        ...
        if self.training:
            ...
            communication_mask = torch.where(communication_maps > -10, ones_mask, zeros_mask).reshape(V, 1, H, W)
