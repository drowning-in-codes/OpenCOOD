...
from opencood.utils import v2_eval_utils as eval_utils
...
def test_parser():
    ...
    parser.add_argument(
        "--last_epoch", type=int, required=False, help="which epoch to load from"
    )
    parser.add_argument(
        "--fusion_method",
        required=True,
...
def main():
    ...
    assert not (
        opt.show_vis and opt.show_sequence
    ...
    opencood_dataset = build_dataset(
        hypes, visualize=True, train=False, isSim=opt.isSim
    )
    ...
    epoch, model = train_utils.load_saved_model(saved_path, model,getattr(opt,"last_epoch",-1))
    loc_error = 0
    heading_error = 0
    if (
        "wild_setting" in hypes
        and "hypes" in hypes["wild_setting"]
        and hypes["wild_setting"]["loc_err"]
    ):
        if "xyz_std" in hypes["wild_setting"]:
            loc_error = hypes["wild_setting"]["xyz_std"]
        else:
            loc_error = 0
        if "ryp_std" in hypes["wild_setting"]:
            heading_error = hypes["wild_setting"]["ryp_std"]
        else:
            heading_error = 0
    ...
    eval_final_results(result_stat, opt.model_dir)
    eval_final_results(result_stat_short, opt.model_dir, "short")
    eval_final_results(result_stat_middle, opt.model_dir, "middle")
    eval_final_results(result_stat_long, opt.model_dir, "long")
    if opt.show_sequence:
        vis.destroy_window()
