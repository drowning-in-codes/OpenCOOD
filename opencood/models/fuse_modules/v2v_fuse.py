...
class V2VNetFusion(nn.Module):
    def __init__(self, args):
        ...
        self.use_temporal_encoding = args.get("use_temporal_encoding",False)
        self.discrete_ratio = args["voxel_size"][0]
        self.downsample_rate = args["downsample_rate"]
        self.num_iteration = args["num_iteration"]
        self.gru_flag = args["gru_flag"]
        ...
        self.cnn = nn.Conv2d(
            in_channels + 1, in_channels, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, x, record_len, pairwise_t_matrix, prior_encoding):
        ...
        # (B,L,L,2,3)
        pairwise_t_matrix = get_discretized_transformation_matrix(
            pairwise_t_matrix.reshape(-1, L, 4, 4),
            self.discrete_ratio,
            self.downsample_rate,
        ).reshape(B, L, L, 2, 3)
        # (B*L,L,1,H,W)
        roi_mask = get_rotated_roi(
            (B * L, L, 1, H, W), pairwise_t_matrix.reshape(B * L * L, 2, 3)
        )
        roi_mask = roi_mask.reshape(B, L, L, 1, H, W)
