...
class SortSampler(nn.Module):
    ...
    def forward(self, src, pos_embed, sample_ratio, dis_priority):
        bs, c, h, w = src.shape
        # 各位置的分数
        src_dis = dis_priority * src.permute(1, 0, 2, 3)  # bs h w * c bs h w -> c bs h w
        src_dis = src_dis.permute(1, 0, 2, 3).float()  # bs c h w # meaningless behavior? introduce range-aware info
        # print(src_dis.shape)
        sample_weight = self.score_pred_net(src_dis).sigmoid().view(bs, -1)  # bs c h w -> bs 256/32/1 h w -> bs h*w
        ...
        ################################### wtf ??? why just sample_lens = h * w * sample_ratio
        sort_order = sample_weight_clone.sort(descending=True, dim=1)[1]  # 降序
        sort_confidence_topk = sort_order[:, :max_sample_num]  # bs max_sample_num
        ...
        # max_sample_num<h*w bs c
        # h*w bs c ->  h*w bs c

        # bs c * h * w   bs  max_sample_num ->   h * w bs  1
        # h*w bs c
        # max_sample_num bs c
        src_sampled = src.gather(0, sort_confidence_topk.permute(1, 0)[..., None].expand(-1, -1, c)) \
                      * sample_weight.gather(1, sort_confidence_topk).permute(1, 0).unsqueeze(-1)
