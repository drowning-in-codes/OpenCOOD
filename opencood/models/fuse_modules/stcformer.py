...
import numpy as np
from opencood.models.fuse_modules.how2comm_deformable_transformer import RPN_transformer_deformable_mtf_singlescale
...
class Decoupling(nn.Module):
    ...
    def forward(self, feat, confidence):
        ...
        return torch.cat(exclusive_list, dim=0), torch.cat(common_list, dim=0), torch.cat(exclusive_map_list,
                                                                                          dim=0), torch.cat(
            common_map_list, dim=0)
