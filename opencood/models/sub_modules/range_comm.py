...
class RangeComm(nn.Module):
    def __init__(
        ...
        if multi_scale:
            ...
            for idx in range(self.layer_num):
                if with_prior and idx == 0:
                    self.fusion_model.append(
                        lightRangeAttentionFusion(num_filters[idx] + 3)
                    )
                else:
            ...
            if with_prior:
                ...
                model_cfg=fusion_args, input_channels=input_dim+3
            )
            else:
                self.fusion_model = RangeAttentionFusion(
                    model_cfg=fusion_args, input_channels=input_dim
                )
        if with_comm:
            self.mrc = MultiRangeComm(
                input_dim, cls_head=cls_head, config=args, H=H, W=W
            )
    ...
    def forward(
        ...
        if self.multi_scale:
            ...
            for idx in range(self.layer_num):
                ...
                if self.with_comm:
                    if idx == 0:
                        ...
                        # if idx == self.layer_num - 1:
                        #     last_conf_map = conf_map
                        batch_feat = self.regroup(x, record_len)  # [(V,C,H, W),...]
                        for batch in range(B):
                            # B, max_cav, 3(dt dv infra), 1,1
                            communication_mask, communication_rate,comm_map = self.mrc(batch_feat[batch])
                            communication_masks.append(communication_mask)
                            communication_rates.append(communication_rate)
                            communication_maps.append(comm_map)
                        communication_rates = sum(communication_rates) / B
                        communication_masks = torch.cat(communication_masks, dim=0)
                        if x.shape[-1] != communication_masks.shape[-1]:
                            communication_masks = F.interpolate(
                ...
                x_fuse = self.fusion_model[idx](x, record_len, distance)
            ...
            if len(backbone.deblocks) > self.layer_num:
            ...
            if self.with_comm:
                ...
                batch_feat = self.regroup(x, record_len)
                for batch in range(B):
                    # B, max_cav, 3(dt dv infra), 1,1
                    communication_mask, communication_rate,comm_map = self.mrc(batch_feat[batch])
            ...
            # communication feat
            communication_feat = x
            if self.with_prior and prior_encoding_stack is not None:
        ...
        return (
            ...
            communication_feat,
            communication_maps if self.with_comm else None,
            communication_masks if self.with_comm else None,
        )
