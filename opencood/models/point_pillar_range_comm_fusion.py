...
class PointPillarRangeCommFusion(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        prior_encoding_stack = torch.cat(
            prior_encoding_stack, dim=0
        )  # (sum(cavvs),3,1,1)
        batch_dict = {
        ...
        # communication
        prior_encoding_stack = prior_encoding_stack.repeat(
            1, 1, spatial_features_2d.shape[2], spatial_features_2d.shape[3]
        )

        fused_feature, communication_rates, communication_feat = self.comm(
            spatial_features_2d,
            record_len=record_len,
            distance=distance_to_ego,
