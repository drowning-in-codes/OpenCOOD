...
from opencood.models.sub_modules.range_comm import RangeComm
...
class PointPillarRangeCommFusion(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args["compression"] > 0:
            ...
            if self.shrink_flag:
                self.compressor = NaiveCompressor(
                    args["shrink_header"]["dim"][-1], args["compression"]
                )
            else:
        ...
        self.cls_head = nn.Conv2d(
            output_fusion_feature_dim, args["anchor_number"], kernel_size=1
        )
        self.reg_head = nn.Conv2d(
            output_fusion_feature_dim, 7 * args["anchor_number"], kernel_size=1
        )
        with_comm = (
            args["communication"]["with_comm"]
            if "with_comm" in args["communication"]
            else False
        )
        self.comm = RangeComm(
            H=100,
            ...
            args=args["communication"],
            fusion_args=args["raa_fusion"],
    ...
    def forward(self, data_dict,keep_grad=False,with_prior =False):
        ...
        # communication
        prior_encoding_stack = prior_encoding_stack.repeat(1, 1, spatial_features_2d.shape[2],
        ...
        if self.multi_scale:
            fused_feature, communication_rates, communication_feat,comm_map,comm_mask = self.comm(
            ...
            fused_feature, communication_rates, communication_feat,comm_map,comm_mask = self.comm(
                spatial_features_2d, record_len=record_len, distance=distance_to_ego,
                prior_encoding_stack=prior_encoding_stack
            )
            # downsample feature to reduce memory
            if self.shrink_flag:
                fused_feature = self.shrink_conv(fused_feature)
