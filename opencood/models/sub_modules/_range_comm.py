...
class RangeComm(nn.Module):
    def __init__(
        ...
        self.threshold = (
            1e-2 if args is None or "threshold" not in args else args["threshold"]
        )
        self.lam = nn.Parameter(torch.tensor(ratio))
        self.mrc = MultiRangeComm(input_dim, cls_head=cls_head, config=None, H=H, W=W)
    ...
    def forward(self, x: torch.Tensor, record_len, backbone=None, distance=None):
        # TODO add multi_scale confidence map(imporance map) and change the communication generation scheme. with range.
        ...
        if self.multi_scale:
            ...
            for idx in range(self.layer_num):
                ...
                if self.with_comm:
                    if idx == 0:
                        ...
                        for batch in range(B):
                            communication_mask, communication_rate,communication_map = self.mrc(
                                batch_feat[batch]
                            )
                            communication_masks.append(communication_mask)
                            communication_rates.append(communication_rate)
            ...
            if self.with_comm:
                ...
                for batch in range(B):
                    communication_mask, communication_rate,communication_map = self.mrc(batch_feat[batch])
                    communication_masks.append(communication_mask)
                    communication_rates.append(communication_rate)
                    communication_maps.append(communication_map)
                communication_rates = sum(communication_rates) / B
