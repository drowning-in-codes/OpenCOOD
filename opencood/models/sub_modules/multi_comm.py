...
class Naive_DCN(nn.Module):
    ...
    def forward(self, feat):
        ...
        q_lt = torch.cat(
            [torch.clamp(q_lt[..., :N], 0, feat.size(2) - 1), torch.clamp(q_lt[..., N:], 0, feat.size(3) - 1)],
            dim=-1).long()
        q_rb = torch.cat(
            [torch.clamp(q_rb[..., :N], 0, feat.size(2) - 1), torch.clamp(q_rb[..., N:], 0, feat.size(3) - 1)],
            dim=-1).long()
        q_lb = torch.cat([q_lt[..., :N], q_rb[..., N:]], dim=-1)
        q_rt = torch.cat([q_rb[..., :N], q_lt[..., N:]], dim=-1)

        # clip p
        p = torch.cat([torch.clamp(p[..., :N], 0, feat.size(2) - 1), torch.clamp(p[..., N:], 0, feat.size(3) - 1)],
                      dim=-1)
...
class Convrange(nn.Module):
    def __init__(self, in_channels, out_channels=None, kernel_size=3, stride=1, padding=1, groups=1, range_type="near",
                 H=None, W=None):
        ...
        self.convmodule = nn.Sequential(
            nn.Conv2d(in_channels + 3, out_channels, kernel_size=kernel_size, stride=stride, padding=padding,
                      groups=groups),
    ...
    def forward(self, x):
        ...
        grid_h_batch, grid_w_batch, distance_tensor = grid_h_batch.unsqueeze(0).unsqueeze(0).repeat(v, 1, 1, 1), \
            grid_w_batch.unsqueeze(0).unsqueeze(0).repeat(v, 1, 1, 1), distance_tensor.unsqueeze(0).unsqueeze(0).repeat(
            v, 1, 1, 1)
...
class MultiRangeComm(nn.Module):
    ...
    def forward(self, batch_features):
        ...
        for conv, n_conv, f_conv in zip(self.down_convs, self.n_convs, self.f_convs):
            ...
            # plt.imshow(comm_map[0].detach().cpu().numpy())
            # plt.colorbar()
            # plt.axis("off")
            # plt.show()
        ...
        # plt.imshow(comm_maps[-1].squeeze().detach().cpu().numpy())
        # plt.colorbar()
        # plt.show()
        # plt.imshow(communication_mask[-1].squeeze().detach().cpu().numpy(), vmin=0, vmax=1)
        # plt.colorbar()
        # plt.show()
        ...
        if self.training:
            ...
            communication_mask = torch.where(communication_maps > self.threshold, ones_mask, zeros_mask).reshape(V, 1,
                                                                                                                 H, W)
        ...
        # plt.imshow(communication_maps[-1].squeeze().detach().cpu().numpy())
        # plt.colorbar()
        # plt.show()

        return communication_mask, communication_rate.item(),communication_maps
