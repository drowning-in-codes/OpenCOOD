...
class CrossAttentionWithEmbedding(nn.Module):
    ...
    def forward(self,query,key,value,pos_embedding=None):
        # [1,C,H,W]
        assert query.shape[0] == 1 and key.shape[0] == 1 and value.shape[0] == 1,"batch size must be 1"
        assert query.ndim == 4 and key.ndim == 4 and value.ndim == 4,"tensor dim not correct"
        dim = query.shape[1]
        query = self.to_q(query) # [cav_num,C,H,W]
        key = self.to_k(key) # [cav_num,C,H,W]  
        value = self.to_v(value) # [cav_num,C,H,W]
        query = rearrange(query, "b (c h) x y -> b h (x y) c",h=self.heads) # [1,heads,H*Wvisio,C]
        if pos_embedding is not None:
            ...
            context = torch.matmul(query,context.transpose(-1,-2)) # [cav_num,heads,H*W,H*W]
        key = rearrange(key, "b (c h) x y -> b h (x y) c",h=self.heads) # [1,heads,H*W,C]
        if self.use_distance:
            value = self.raan(value)
        value = rearrange(value, "b (c h) x y -> b h (x y) c",h=self.heads) # [1,heads,H*W,C]
        ...
        x = query.shape[2]
        out = rearrange(out, "b h (x y) c -> b (c h) x y",h=self.heads,x=x) # [1,C,H,W]
        out = self.W_o(out)
        return out


class CrossAgentChannelAttention(nn.Module):
    # !FIX cross attention with rel pos encoding
    def __init__(self,input_dim,factor:int=2) -> None:
        super().__init__()
        self.factor = factor
        self.to_qkv = nn.ConvTranspose2d(input_dim, input_dim*3, kernel_size=factor,stride=factor, bias=True)
        self.W_o = nn.Conv2d(input_dim, input_dim, kernel_size=factor,stride=factor, bias=True)

    def forward(self,batch_spatial_feature,distance=None):
        # params [V,C,H,W]
        ...
        q, k, v = map(lambda t: rearrange(t, "b c (x_h x) (y_h y) -> b (x_h y_h) c (x y)", x_h=self.factor,y_h=self.factor), qkv)
        # [cav_num,num_head,C,H*W]   [cav_num,num_head,H*W,C]-> [cav_num,num_head,C,C] 
        score = torch.matmul(q, k.transpose(-2, -1)) / np.sqrt(C) # [cav_num,C,C]
...
class CrossAgentAttention(nn.Module):
    def __init__(self,input_dim,heads:int=3) -> None:
        super().__init__()   
        inner_dim = input_dim * heads
        self.inner_dim = inner_dim
        self.heads = heads
        self.to_qkv = nn.Conv2d(input_dim, inner_dim*3, kernel_size=1, bias=True)
        self.W_o = nn.Conv2d(inner_dim, input_dim, kernel_size=1, bias=True)
    ...
    def forward(self,batch_spatial_feature,distance=None):
        ...
        q, k, v = map(lambda t: rearrange(t, "b (h c) x y -> (x y) h b c", h=self.heads), qkv)
        score = torch.matmul(q, k.transpose(-2, -1)) / np.sqrt(cav_num) # [V,C,C]
        attn = F.softmax(score, -1) #  [H*W,head,cav_num,dim]
        out = torch.matmul(attn, v) #  [H*W,head,cav_num,cav_num] [H*W,head,cav_num,dim]-> [H*W,head,cav_num,dim]
        out = out.permute(2,1,3,0).reshape(cav_num,self.inner_dim,H,W) # [H*W,cav_num,C]
        out = self.W_o(out)
        return out
...
class CrossAgentWiseAttention(nn.Module):
    def __init__(self,input_dim) -> None:
        super().__init__()
        self.ca = CrossAgentAttention(input_dim)
        self.cca = CrossAgentChannelAttention(input_dim)

    def forward(self,spatial_features):
        # params spatial_features: [V,C,H,W]
        assert spatial_features.ndim == 4,"tensor dim not correct"
        out = self.ca(spatial_features)
        out = self.cca(out+spatial_features)
        return out+spatial_features
        # ego_feat = spatial_features[0:1]
        # feats = []
        # pos_embedding = self.get_pos_embedding(spatial_features)
        # for i in range(spatial_features.shape[0]):
        #     feat = self.ca(ego_feat,spatial_features[i:i+1],spatial_features[i:i+1],pos_embedding)
        #     feats.append(feat)
        # feats = torch.cat(feats,dim=0)
        # return feats
    
    @staticmethod
    def get_pos_embedding(spatial_features):
        H,W = spatial_features.shape[-2:]
        lin_h = torch.linspace(0, H - 1, H).to(spatial_features)
        lin_w = torch.linspace(0, W - 1, W).to(spatial_features)
        ...
        y = y / float(H // 2)
        x = x / float(W // 2)
        distance_tensor = torch.sqrt(x ** 2 + y ** 2).unsqueeze(0)
...
class CoordinateAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out_attn = identity * a_w * a_h * c_weight
        return out_attn


# class LocalRangeAttention(nn.Module):
#     def __init__(self,depth:int,input_dim:int) -> None:
#         super().__init__()
#         self.depth = depth
#         self.lrblock = nn.ModuleList()
#         hidd_dim = input_dim
#         for i in range(depth):
#             lra = LocalRangeAttentionBlock(input_dim=hidd_dim,last_attention=True if i == depth -1 else False)
#         self.lrblock.append(lra)

#     def forward(self,x):
#         # param x: [1,C,H,W]
#         for lra in self.lrblock:
#             x = lra(x)
#         return x

class CoordinateRangeAttentionBlock(nn.Module):
    def __init__(self, input_dim,output_dim=None,scale:float=1.,last_attention:bool=True) -> None:
        super().__init__()
        assert input_dim % 2 == 0,"channel must be even number"


    def forward(self,batch_featuers):
        pass
...
class LocalInvertedAttention(nn.Module):
    def __init__(self,input_dim):
        super().__init__()
        self.conv1 = LocalRangeAttentionBlock(input_dim)
        self.conv2 = LocalRangeAttentionBlock(input_dim)
        self.dwise = LocalRangeAttentionBlock(input_dim,kernel_size=3,padding=1,groups=input_dim//2)
...
class LocalRangeAttentionBlock(nn.Module):
    def __init__(self, input_dim,output_dim=None,scale:float=1.,kernel_size=1,padding=0,groups=1,last_attention:bool=True):
        super().__init__()
        assert input_dim % 2 == 0,"channel must be even number"
        ...
        self.conv_a = nn.Sequential(
                nn.Conv2d(input_dim, branch_out_channel, kernel_size=kernel_size, padding=padding,groups=groups,bias=True),
        ...
        self.conv_attn_a = nn.Sequential(
            nn.Conv2d(in_channels=branch_out_channel + 2, out_channels=1, kernel_size=kernel_size,padding=padding),
            nn.BatchNorm2d(1),
            nn.ReLU()
        )
        ...
        self.conv_attention_b = nn.Sequential(nn.Conv2d(4,1,kernel_size=3,padding=1),
                                            nn.BatchNorm2d(1),
                                              nn.Sigmoid())
...
class LocalAttention(nn.Module):
    ...
    def __init__(self, input_dim):
        super().__init__()
        self.lsg =  LocalSandGlassAttention(input_dim=input_dim)
        # self.lia = LocalInvertedAttention(intput_dim=input_dim)
        
    def forward(self, spatial_features):  # [1,C,H,W]
        out = self.lsg(spatial_features)
        # out = self.lia(spatial_features)
        return out
...
class RangeAttentionBlock(nn.Module):
    def __init__(self, input_dim):
        ...
        self.gen_local = nn.Conv2d(input_dim, input_dim, kernel_size=3, padding=1, groups=input_dim)
    ...
    def forward(self, spatial_features, record_len):
        ...
        for idx,batch_spatial_feature in enumerate(split_x): # [2,C,H,W]
            ...
            agent_feature = self.agent_att(local_feat) # [V,C,H,W] V is sum of cavs in the scene which can be detected
            ...
            local_weight = torch.sum(self.gen_local(local_max_feat),dim=1,keepdim=True) # [1,1,H,W]
            ...
            agent_weight = self.gen_agent(agent_max_feat)  # [1,C,1,1]

            out_feature = local_weight*local_max_feat + agent_weight*agent_max_feat + ego_feat
