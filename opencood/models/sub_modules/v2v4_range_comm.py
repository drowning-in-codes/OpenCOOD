...
class RangeComm(nn.Module):
    def __init__(
        ...
        if with_comm:
            self.mrc = MultiRangeComm(input_dim, cls_head=cls_head, config=args, H=H, W=W)
    ...
    def forward(self, x: torch.Tensor, record_len, backbone=None, distance=None):
        ...
        if self.multi_scale:
            ...
            for idx in range(self.layer_num):
                ...
                if self.with_comm:
                    if idx == 0:
                        ...
                        for batch in range(B):
                            communication_mask, communication_rate = self.mrc(
                                batch_feat[batch]
                            )
                            communication_masks.append(communication_mask)
                            communication_rates.append(communication_rate)
            ...
            if self.with_comm:
                ...
                for batch in range(B):
                    communication_mask, communication_rate,_ = self.mrc(batch_feat[batch])
                    communication_masks.append(communication_mask)
                    communication_rates.append(communication_rate)
                communication_rates = sum(communication_rates) / B
                communication_masks = torch.cat(communication_masks, dim=0)
